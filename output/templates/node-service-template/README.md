...
Scaffolds a Node.js microservice with Express, TypeScript, Docker and Backstage integration.

## Purpose

This template generates a production-ready Node.js microservice skeleton including:
- TypeScript project structure (`package.json`, `tsconfig.json`)
- Express application boilerplate with health check endpoint
...
This template is consumed via the Backstage Developer Portal. Navigate to **Create → Node.js Microservice** and fill in the form.
...
- **Contribution**: changes must pass the R01–R19 lint check (`./scripts/lint.sh output/templates/node-service-template/`). Breaking changes follow Semantic Versioning — see `CHANGELOG.md`.
