...
if curl -fsSLk --max-time 30 "$URL" -o /tmp/glab.deb 2>/dev/null; then
  sudo dpkg -i /tmp/glab.deb
  rm /tmp/glab.deb
  echo "✅ $(glab version) installé"
else
  echo "⚠️  glab non installé (gitlab.com inaccessible depuis ce réseau)"
fi
...
if curl -fsSLk --max-time 10 "$GH_URL" -o /tmp/gh.deb 2>/dev/null; then
...
# -k : proxy SSL entreprise (inspection TLS)
echo "→ Installation de GitHub Copilot CLI (nouveau binaire standalone)"
if curl -fsSLk --max-time 30 https://gh.io/copilot-install | bash; then
