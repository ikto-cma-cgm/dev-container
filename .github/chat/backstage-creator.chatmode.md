...
1. **DO NOT create a `skeleton-app/`** — this duplicates the base template and breaks C01.
...
3. **Create `skeleton-overlay/`** with only the delta files vs the base skeleton:
   ...
   - **NOT** files identical to the base

4. **Apply the overlay** as a subsequent step with `targetPath: .`

5. For **secondary components** (e.g. Liquibase): remote base fetch with `targetPath`, then `skeleton-<role>-overlay/` with only stub `catalog-info.yaml` and `README.md`.
...
C01 **must PASS** (not SKIP) for any template with remote fetches.
