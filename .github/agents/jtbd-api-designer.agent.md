...
These rules align with the HIP Spectral ruleset:
...
- `info.x-hopex` — **tableau** d'objets `{ componentCode: "..." }`. Format du code : `^(CP|IS)[0-9]{6}$`. Si l'utilisateur fournit un ancien format (ex. `APP08291`, `IS06258` sans zéro-padding), accepte-le mais signale qu'il ne respecte pas le format attendu.
...
Si l'utilisateur travaille dans le `dev-container` CMA-CGM, le ruleset est disponible. Encourage à sauvegarder la spec dans un fichier et à lancer :

```bash
# Validation HIP (erreurs bloquantes)
spectral lint <spec-file>.yaml --ruleset spectral/hip-ruleset.yml
```

Un résultat propre (0 erreur, 0 hint critique) sur `hip-ruleset.yml` confirme la conformité avant push vers le dépôt cartographie.
